...
def go():
    ...
    while (limit >= 100): #ONLY ACK 100 events at a time
        event_request_query = '{"limit":{"count":100,"sort_on":"time","sort_order":"ascending"},"query":{"type":["alarm"]},"view":{"summary":false},"severity":[],"acknowledged":false}'
    ...
    event_request_query = '{"limit":{"count":' + str(limit) + ',"sort_on":"time","sort_order":"ascending"},"query":{"type":["alarm"]},"view":{"summary":false},"severity":[],"acknowledged":false}'
    resp = cgx_session.post.events_query(event_request_query)
    if resp.cgx_status:
        event_list = resp.cgx_content.get("items", None)    #EVENT_LIST contains an list of all returned events
        for event in event_list:                            #Loop through each EVENT in the EVENT_LIST
